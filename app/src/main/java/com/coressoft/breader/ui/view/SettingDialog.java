...
public class SettingDialog extends Dialog {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //初始化亮度
        isSystem = SPUtils.getInstance().isSystemLight();
        setTextViewSelect(tv_xitong, isSystem);
        setBrightness(SPUtils.getInstance().getLight());

        //初始化字体大小
        currentFontSize = (int) SPUtils.getInstance().getFontSize(getContext());
        tv_size.setText(currentFontSize + "");

        //初始化字体
        tv_default.setTypeface(SPUtils.getInstance().getTypeface(getContext(),Config.FONTTYPE_DEFAULT));
        tv_qihei.setTypeface(SPUtils.getInstance().getTypeface(getContext(),Config.FONTTYPE_QIHEI));
        tv_fzkatong.setTypeface(SPUtils.getInstance().getTypeface(getContext(),Config.FONTTYPE_FZKATONG));
        tv_bysong.setTypeface(SPUtils.getInstance().getTypeface(getContext(),Config.FONTTYPE_BYSONG));
        selectTypeface(SPUtils.getInstance().getTypefacePath());

        selectBg(SPUtils.getInstance().getBookBgType());
    ...
    public void setBookBg(int type) {
        SPUtils.getInstance().setBookBg(type);
    ...
    public void setTypeface(String typeface) {
        SPUtils.getInstance().setTypeface(getContext(),typeface);
        Typeface tface = SPUtils.getInstance().getTypeface(getContext(),typeface);
        if (mSettingListener != null) {
    ...
    private void addFontSize() {
        if (currentFontSize < FONT_SIZE_MAX) {
            currentFontSize += 1;
            tv_size.setText(currentFontSize + "");
            SPUtils.getInstance().setFontSize(currentFontSize);
            if (mSettingListener != null) {
    ...
    public void changeBright(Boolean isSystem, int brightness) {
        ...
        setTextViewSelect(tv_xitong, isSystem);
        SPUtils.getInstance().setSystemLight(isSystem);
        SPUtils.getInstance().setLight(light);
        if (mSettingListener != null) {
