...
import com.coressoft.breader.adapter.MarkAdapter;
...
public class BookMarkFragment extends BaseFragment {
    ...
    private List<Marks> bookMarksList;
...
//        bookMarksList = DataSupport.where("bookpath = ?", bookpath).find(BookMarks.class);
            ...
            public boolean onItemLongClick(AdapterView<?> parent, View view, final int position, long id) {
                new AlertDialog.Builder(getActivity())
                        ...
                        .setPositiveButton("删除", new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialog, int which) {
                                DataSupport.delete(Marks.class,bookMarksList.get(position).getId());
                                bookMarksList.clear();
                                bookMarksList.addAll(DataSupport.where("bookpath = ?", bookpath).find(Marks.class));
     ...
     * @return
     */
    public static BookMarkFragment newInstance(int bookpath)
    {
        Bundle bundle = new Bundle();
        bundle.putInt(ARGUMENT, bookpath);
